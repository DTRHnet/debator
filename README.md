...
No OpenRouter secret belongs in a repository or client-side environment variable. After signing in, open **Settings**, create an API key from the [OpenRouter key page][1], and paste it into the optional personal-key field. The server encrypts the key with the project session secret before database storage; the browser retains only the final four characters for confirmation.

Choose a current OpenRouter model slug ending in `:free`, such as the default shown in Settings. DebateRush first asks OpenRouter for a strict JSON-schema response and validates it server-side. If a free endpoint does not support structured output parameters, the proxy makes a second constrained JSON request and validates the response before saving it. OpenRouter authenticates API calls with a bearer token and accepts requests at its chat-completions endpoint.[2] Its structured-output feature is configured through `response_format` with a JSON schema, though availability differs between model endpoints.[3]
